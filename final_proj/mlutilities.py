...
def partition(X,T,trainFraction,shuffle=False,classification=False):
    ...
    if not classification:
        ...
        for c in classes:
            ...
            testIndices += rowIndices[cRows[nTrain:nTrain + nTest]].tolist()
...
### from Nabney's netlab matlab library
    ...
    return {'x':x, 'f':fnow, 'nIterations':j-1, 'xtrace':xtrace[:j,:] if xtracep else None, 'ftrace':ftrace[:j],
            'reason':"completed iterations"}
...
def steepest(x, f, gradf, *fargs, **params):
    ...
    return {'x':newx, 'f':newf, 'nIterations':i-1, 'xtrace':xtrace[:i,:], 'ftrace':ftrace[:i], 'reason':"completed iterations"}
